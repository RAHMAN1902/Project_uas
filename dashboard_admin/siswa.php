...
$students = $conn->query("
    SELECT * 
    FROM students 
    ORDER BY FIELD(class, 'Bahasa Indonesia', 'Bahasa Inggris', 'Biologi', 'Seni Budaya' , 'Penjas', 'Agama', 'IPS', 'Kimia ', 'PPKN') 
        ...
        <div class="flex-1 p-6">
            ...
            <div class="mt-4 flex justify-between items-center">
                <a href="?page=<?php echo max($page - 1, 1); ?>"
                    class="bg-sky-950 text-white px-4 py-2 rounded <?php echo $page == 1 ? 'opacity-50 pointer-events-none' : ''; ?>">Previous</a>
                <span>Halaman <?php echo $page; ?> dari <?php echo $totalPages; ?></span>
                <a href="?page=<?php echo min($page + 1, $totalPages); ?>"
                    class="bg-sky-950 text-white px-4 py-2 rounded <?php echo $page == $totalPages ? 'opacity-50 pointer-events-none' : ''; ?>">Next</a>
            </div>
        </div>
    </div>
